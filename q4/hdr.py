...
def build_hdr(imgs, log_t, lam=100, samples=100):
    ...
    gR, gG, gB = (
        gsolve(Z(0), log_t, lam),
        gsolve(Z(1), log_t, lam),
        gsolve(Z(2), log_t, lam),
    )
...
if __name__ == "__main__":
    ...
    # UPDATED FILE NAMES
    files = ["s1.jpg", "s2.jpg", "s3.jpg"]

    imgs = resize_all([imageio.imread(f) for f in files])

    # Example shutter speeds (you can adjust if needed)
    ...
    imageio.imwrite("op.jpg", ldr)
    print("Saved output as op.jpg")
